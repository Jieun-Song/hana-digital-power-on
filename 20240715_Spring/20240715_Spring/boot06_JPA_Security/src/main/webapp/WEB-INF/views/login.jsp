...
<!-- jquery.com에서 제공하는 최신 jQuery 라이브러리 읽어오는 CDN 방식 -->
<script type = "text/javascript">
	...
	function pwd_find(){
		...
		/* 
		자바스크립트의 window객체 하위의 open(alert창 경로, alert창 이름, alert창 속) 매서드를 사용해
		폭이 400px, 높이가 300px, 스크롤바가 생성되는 새로운 alert창을 만든다.
		*/
	}
</script>
</head>
	...
	<div id = "Login_wrap">
		...
		<div style="margin : 0px 0px 0px 50px;">
			<c:if test = "${param.error != null}">
				...
				<!-- JSTL 에서 <c:out value = " "/>는 값을 출력할 떄 사용 -->
			</c:if>
		</div>
		<div style = "margin : 0px 0px 0px 50px;">
		...
		<!-- form 태그에서 action 속성을 생략하면 이전 매칭주소가 action 속성값이 된다.
			결국 login 매핑주소가 action 속성값이 된다.
			스프링 시큐리티가 적용되면 post 방식으로 보내는 모든 데이터에는 CSRF 토큰값이 필요하다.  -->
			<table id = "Login_t">
				<tr>
					<th>아이디</th>
					<td><input name = "username" id = "username" size = "14" tabindex="1" placeholder="아이디" /></td>
					<!-- tabindex="1"로 지정하면 탭키를 눌렀을떄 첫번째로 포커스를 가진다. input태ㄱ에서
					type속성을 생략하면 기본값은 text이다. -->
					<th rowspan = "2">
						<input type = "hidden" name = "${_csrf.parameterName}" value= "${_csrf.token}"/>
					<!-- _csrf 속성은 사이트 간 요청 위조를 방지하기 위한 것으로, 요청을 보내는 URI에서
					서버가 가지는 동일한 값과 같은 값을 가지고 데이터를 전송할 때만 신뢰하는 방법이다. -->
