...
@RestController
@RequestMapping("/replies") //컨트롤러 자체 매핑주소 replies등
...
//		@RequestBody 에노테이션은 전송된 키,값 쌍의 json 데이터를 ReplyVO 객체타입으로 변환해준다.
		ResponseEntity<String> entity = null;
		try {
			this.replyService.insertReply(vo); //댓글 등록
			entity = new ResponseEntity<>("SUCCESS",HttpStatus.OK);//댓글저장 성공시 SUCCESS문자와 200정상상태코드 
		}catch(Exception e) {
			e.printStackTrace();
			entity = new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
			//예외 에러가 발생하면 에러메세지와 나쁜 상태 코드가 반환됨.
		}
		return entity;
	}//addReply()
	...
	public ResponseEntity<List<ReplyVO>> getReplyList(@PathVariable("bno") int bno){
		//@PathVariable("bno")는 매핑주소로부터 bno 게시판 번호값을 추출하는 용도로 활용된다.
		
		ResponseEntity<List<ReplyVO>> entity = null;
		try {
			entity = new ResponseEntity<>(this.replyService.listReply(bno),HttpStatus.OK);//게시판 번호에 해당하는 댓글 목록 반
		}catch(Exception e){
			e.printStackTrace();
	...
	public ResponseEntity<String> editReply(@PathVariable("rno") int rno, @RequestBody ReplyVO vo){
		...
		try {
			...
			this.replyService.updateReply(vo);//댓글 수
	...
	public ResponseEntity<String> del(@PathVariable("rno") int rno){
		...
		try {
			this.replyService.deleteReply(rno);//댓글번호를 기준으로삭제
