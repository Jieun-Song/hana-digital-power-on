...
public class SampleController {
	...
	public ModelAndView start_thymeLeaf() {
		...
		tm.addObject("greeting", "반갑습니다 뷰템플릿 엔진 타임리프");
		//키,값 쌍으로 저장
		//문자열 키이름에 자바 최상위 클래스 Object타입으로 업캐스팅한 상태에서 저장됨 
		tm.setViewName("./thymeLeaf/start_thymeLeaf");
		//타임리프 뷰페이지 경로 설정 ->/src/main/resources/templates/thymeLeaf/start_thymeLeaf.html 이다.
		//결국 templates폴더 하위에 thymeLeaf폴더를 생성해야한다.
		return tm;
	...
	/* get 방식인 경우 예)
	 * 
	 * 1. 서블릿에서 doGet() 메서드를 오버라이딩 한 경우는 get으로 접근할 때 호출된다.
	 * 2. 웹주소창에서 매핑주소를 직접 입력하는 경우
	 * 3. 자바스크립트의 location 객체에 의해서 이동하는 경우
	 * 4. 하이퍼링크에서 이동하는 경우
	 * 5. 스프링 백엔드컨트롤 다른 매핑주소로 이동할 때 사용하는 redirect:/도 get방식
