...
public class BoardDAOImpl implements BoardDAO {
	...
	public void insertBoard(BoardVO b) {
		...
		//mybatis에서 insert()매서드는 레코드를 저장하고, board_in은 board.xml에서 설정할
		//유일 아이디명이다.
	...
	public int getTotalCount() {
		return this.sqlSession.selectOne("b_count");//mybatis에서 selectOne() 메서드는 단 하나의 레코드를 반환하고 b_count는
		//board.xml에서 설정할 유일 아이디명이다.
	}// 총 레코드 개수 

	@Override
	public List<BoardVO> getBoardList(BoardVO b) {
		return this.sqlSession.selectList("b_list", b); //mybatis에서 selectList()매서드는 하나이상의 레코드를 반환한다.b_list는
		//board.xml에서 설정할 유일 아이디명이다.
	}// 게시물 목록 

	@Override
	public void updateHit(int bno) {
		this.sqlSession.update("b_hit", bno);//mybatis에서 update()메서드는 레코드를 수정한다.
		//b_hit는 board.xml에서 설정할 유일 아이디명이다.
