...
public class BoardController {
	...
	public void board_write(HttpServletRequest request, Model m) {
		...
		m.addAttribute("page",page);//page키이름에 쪽번호를 저장 => 페이징에서 내가 본 쪽번호로 바로 이동하기 위한
		//책갈피 기능 구현 
	}//board_write()
...
//		페이징연산
		...
		int startpage = (((int)((double)page/10+0.9))-1)*10+1;//현재 페이지에 보여지는 시작페이지
		int endpage = maxpage;//현재페이지에 보여질 마지막 페이지
		
		if (endpage > startpage+10-1) endpage = startpage+10-1;
		
		ModelAndView listM = new ModelAndView("board/board_list");//생성자 인자값으로 뷰페이지 경로설정
		listM.addObject("totalCount", totalCount); //totalCount키 이름에 총 게시물 수 저장 
	...
	//게시판 내용보기와 조회수 증가, 수정폼
	@RequestMapping("/board_cont") //get or post로 접근하는 매팅주소를 처리, board_cont 매핑주소 등록
	public ModelAndView board_cont(@RequestParam("bno") int bno, int page, String state) {
		BoardVO bc = null;
		
		if(state.equals("cont")) {//내용보기일때 실행 
			bc = this.boardService.getBoardCont(bno);//내용보기와 조회수 증가 ->내용보기일때만 조회수 증
		}else { //내용보기가 아닌 수정폼
			bc = this.boardService.getBoardCont2(bno);//조회수 증가가 안된 수정
		}
		
		ModelAndView cm = new ModelAndView();
		cm.addObject("page", page);//페이징에서 내가 본 쪽번호로 바로 이동하기 위한 책갈피 기능 구현을 위해 쪽번호저장
		cm.addObject("bc", bc);
		
		if(state.equals("cont")) {//내용보기 뷰페이지
			//cm.setViewName("board/board_cont");//뷰리졸브(화면이 보여지는뷰페이지) 경로는 
			///WEB-INF/views/board/board_cont.jsp
			cm.setViewName("board/board_cont");
			//모든 게시물 내용에 대한 댓글 기능이 되게 만들어보자.
			//test.jsp파일 내용을 board_cont2.jsp와 결합시키면 된다.
